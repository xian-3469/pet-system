...
if port_up 6379; then echo "[1/4] Redis 已在运行"; else
  (redis-server --port 6379 >/tmp/redis_run.log 2>&1 &) ; echo "[1/4] Redis 已启动"
fi
...
if port_up 3306; then echo "[2/4] MySQL 已在运行"; else
  ...
  ("$MYSQL_BASE/bin/mysqld" --no-defaults --basedir="$MYSQL_BASE" --datadir="$HOME/tools/mysql-data" \
    --socket="$HOME/tools/mysql.sock" --port=3306 --bind-address=0.0.0.0 \
    --pid-file="$HOME/tools/mysql.pid" --tmpdir="$HOME/tools/mysql-tmp" >/tmp/mysql_run.log 2>&1 &)
...
if port_up 9311; then echo "[3/4] 后端已在运行"; else
  cd "$API_DIR" && nohup mvn -q spring-boot:run -DskipTests >/tmp/backend.log 2>&1 &
  echo "[3/4] 后端启动中（约 40 秒），日志 /tmp/backend.log"
fi

# 4. 前端（9312）
if port_up 9312; then echo "[4/4] 前端已在运行"; else
  cd "$VUE_DIR" && NODE_OPTIONS=--openssl-legacy-provider nohup npx vue-cli-service serve >/tmp/frontend.log 2>&1 &
