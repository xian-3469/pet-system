...
<!--      <el-table-column prop="pet" label="宠物"></el-table-column>-->
      ...
      <el-table-column label="操作"  fixed="right"   width="640" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="warning" plain @click="showAiAnalysis(scope.row)" :loading="analyzingId === scope.row.id">
            {{ analyzingId === scope.row.id ? '分析中' : 'AI 分析' }}
          </el-button>
          <el-popconfirm
              class="ml-5"
              confirm-button-text='确定'
    ...
    <el-dialog :title="analysisTitle" :visible.sync="analysisVisible" width="560px" :close-on-click-modal="false">
      <div v-if="analysis">
        ...
        <div v-if="analysis.suggestions && analysis.suggestions.length" style="margin-top: 12px">
          <b style="color: #409EFF">💡 审核参考</b>
          <ul style="padding-left: 20px; margin: 6px 0; color: #555; line-height: 1.8">
            <li v-for="(s, i) in analysis.suggestions" :key="'s'+i">{{ s }}</li>
          </ul>
        </div>
        <div style="text-align: center; color: #bbb; font-size: 12px; margin-top: 10px">AI 分析仅供参考，请结合实际情况审核</div>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    changeState(row, state) {
      this.request.post("/applcation/state/" + row.id + "/" + state).then(res => {
        ...
        this.$message.error(err.response?.data?.msg || "请求失败，请稍后重试")
      })
    },
    load() {
    ...
    showAiAnalysis(row) {
      ...
      this.analyzingId = row.id
      this.analysis = null
      this.analysisTitle = "「" + (row.animal ? row.animal.nickname : '') + "」领养申请 AI 分析（申请人：" + row.name + "）"
      this.request.get("/ai/adopt-analysis/" + row.id, { timeout: 120000 }).then(res => {
